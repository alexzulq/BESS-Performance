...
export const generatePPT = (data: SimulationResult, selectedSites: string[], capacityMW: number) => {
  ...
  const createLossTable = (x: number, y: number, w: number, lossData: LossFactors, title: string) => {
      // Card Container
      slide2.addShape(pres.ShapeType.rect, { x: x, y: y, w: w, h: 2.2, fill: { color: 'FFFFFF' }, line: { color: 'E2E8F0' } });
      ...
      const rows = [
          ...
          ['PR Impact', Math.round(lossData.variancePr).toLocaleString(), `${((lossData.variancePr / lossData.kwhBudget)*100).toFixed(1)}%`],
  ...
  if (events.length > 0) {
      ...
      slide5.addTable([eventHeader, ...eventRows] as any[], {
          x: 0.5, y: 1.0, w: '90%',
          fontSize: 10,
          colW: [1.5, 2.5, 4, 1.5],
  ...
  slide6.addTable([tableHeader, ...tableRows] as any[], {
      x: 0.5, y: 1.0, w: '90%',
      fontSize: 10,
      border: { pt: 1, color: 'E2E8F0' },
      fill: { color: 'FFFFFF' },
